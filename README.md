...
image, and also generating syntactically and semantically correct sentences that describe the image. Deep learning-based techniques, which are known to work well for a wide array of complex tasks, have been used for Image Captioning as well. Here, we aim to present a comparative study of the performance of two deep learning-based image captioning techniques on the benchmark [MS COCO](https://cocodataset.org/#home)  dataset. We, thus, aim to compare these models using Natural Language Processing performance metrics such as [BLEU](https://en.wikipedia.org/wiki/BLEU), [GLEU](https://www.nltk.org/_modules/nltk/translate/gleu_score.html), [METEOR](https://en.wikipedia.org/wiki/METEOR) and [WER](https://en.wikipedia.org/wiki/Word_error_rate) (Word Error Rate). Furthermore, we also analyze the performance of various model combinations by varying the parameters (vocabulary size, batch size, number of images, epochs) and convolutional network ([ResNet50](https://keras.io/api/applications/resnet/#resnet50-function), [Inception V3](https://keras.io/api/applications/inceptionv3/)) used.
